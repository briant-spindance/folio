...
- The Go backend reads `forge/.env.local` on startup and exposes an `/api/chat` endpoint that proxies requests to the configured LLM provider via the Vercel AI SDK's server-side streaming utilities.
