...
import { getArtifactRawUrl } from "@/lib/api"
...
function categorize(type: string, name: string): ArtifactCategory {
  ...
  // Text and data files are code-editable
  if (type === "text" || type === "data") return "code"
  // Check extension for code-like files
...
function ArtifactBreadcrumbs({ slug, featureTitle, filename }: { slug: string; featureTitle?: string; filename: string }) {
  return (
    <nav className="artifact-breadcrumbs">
      <Link to="/features" className="artifact-breadcrumb-link">Features</Link>
      <span className="artifact-breadcrumb-sep">&lsaquo;</span>
      <Link to={`/features/${slug}`} className="artifact-breadcrumb-link">{featureTitle ?? slug}</Link>
      <span className="artifact-breadcrumb-sep">&lsaquo;</span>
...
function ImageViewer({ slug, filename }: { slug: string; filename: string }) {
  const rawUrl = getArtifactRawUrl(slug, filename)
  return (
...
export function ArtifactView() {
  ...
  const deleteMutation = useDeleteArtifact(slug ?? "")
  ...
  // Detect this and render the appropriate viewer.
  if (error && filename) {
            ...
            <div className="artifact-view-header">
              <div className="artifact-view-title-row">
                ...
                <div className="artifact-view-actions">
                  <a
                    href={getArtifactRawUrl(slug ?? "", filename)}
                    download={filename}
  ...
  if (!artifact) {
    return (
      <div>
        <ArtifactBreadcrumbs slug={slug ?? ""} featureTitle={feature?.title} filename={filename ?? "Unknown"} />
  ...
  return (
    <div>
      <ArtifactBreadcrumbs slug={slug ?? ""} featureTitle={feature?.title} filename={artifact.name} />
        ...
        <div className="artifact-view-header">
          <div className="artifact-view-title-row">
            ...
            <div className="artifact-view-actions">
              ...
              <a
                href={getArtifactRawUrl(slug ?? "", artifact.name)}
